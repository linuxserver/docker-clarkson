pipeline {
  ...
  environment {
    ...
    EXT_REPO = 'Clarkson'
    BUILD_VERSION_ARG = 'CLARKSON_RELEASE'
    LS_USER = 'linuxserver'
    LS_REPO = 'docker-clarkson'
    CONTAINER_NAME = 'clarkson'
    DOCKERHUB_IMAGE = 'linuxserver/clarkson'
    DEV_DOCKERHUB_IMAGE = 'lsiodev/clarkson'
    PR_DOCKERHUB_IMAGE = 'lspipepr/clarkson'
    DIST_IMAGE = 'alpine'
    MULTIARCH='true'
    CI='true'
    CI_WEB='true'
  ...
  stages {
    ...
    stage("Scarf.sh package registry"){
      ...
      steps{
        sh '''#! /bin/bash
              set -e
              PACKAGE_UUID=$(curl -X GET -H "Authorization: Bearer ${SCARF_TOKEN}" https://scarf.sh/api/v1/organizations/linuxserver-ci/packages | jq -r '.[] | select(.name=="linuxserver/clarkson") | .uuid')
              if [ -z "${PACKAGE_UUID}" ]; then
                echo "Adding package to Scarf.sh"
                curl -sX POST https://scarf.sh/api/v1/organizations/linuxserver-ci/packages \
                  -H "Authorization: Bearer ${SCARF_TOKEN}" \
                  -H "Content-Type: application/json" \
                  -d '{"name":"linuxserver/clarkson",\
                       "shortDescription":"example description",\
                       "libraryType":"docker",\
                       "website":"https://github.com/linuxserver/docker-clarkson",\
                       "backendUrl":"https://ghcr.io/linuxserver/clarkson",\
                       "publicUrl":"https://lscr.io/linuxserver/clarkson"}' || :
              else
    ...
    stage('Build-Single') {
      ...
      steps {
        ...
        sh "docker build \
          ...
          --label \"org.opencontainers.image.url=https://github.com/linuxserver/docker-clarkson/packages\" \
          --label \"org.opencontainers.image.documentation=https://docs.linuxserver.io/images/docker-clarkson\" \
          --label \"org.opencontainers.image.source=https://github.com/linuxserver/docker-clarkson\" \
          --label \"org.opencontainers.image.version=${EXT_RELEASE_CLEAN}-ls${LS_TAG_NUMBER}\" \
          --label \"org.opencontainers.image.revision=${COMMIT_SHA}\" \
          --label \"org.opencontainers.image.vendor=linuxserver.io\" \
          --label \"org.opencontainers.image.licenses=GPL-3.0-only\" \
          --label \"org.opencontainers.image.ref.name=${COMMIT_SHA}\" \
          --label \"org.opencontainers.image.title=Clarkson\" \
          --label \"org.opencontainers.image.description=[Clarkson](https://github.com/linuxserver/Clarkson) is a web-based dashboard application that gives you a neat and clean interface for logging your fuel fill-ups for all of your vehicles. The application has full multi-user support, as well as multiple vehicles per user. Whenever you fill-up your car or motorcycle, keep the receipt and record the data in Clarkson.\" \
